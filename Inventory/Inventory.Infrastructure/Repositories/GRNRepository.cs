    ...
    public class GRNRepository : IGRNRepository
    {
        ...
        public async Task<GRNPagedResponseDto> GetGRNPagedListAsync(string search, string sortField, string sortOrder, int pageIndex, int pageSize, bool isQuick = false)
        {
            ...
            var projectedQuery = query.Select(g => new GRNListDto
            {
                ...
                TotalActualRejected = g.GRNItems.Where(d => !(d.Rack.Name.ToLower().Contains("e1") || d.Rack.Name.ToLower().Contains("expired") || d.Rack.Name.ToLower().Contains("damaged") || d.Rack.Name.ToLower().Contains("rejected") || d.Rack.Name.ToLower().Contains("purged") || (d.Rack.Description != null && (d.Rack.Description.ToLower().Contains("expired") || d.Rack.Description.ToLower().Contains("damaged") || d.Rack.Description.ToLower().Contains("rejected") || d.Rack.Description.ToLower().Contains("purged"))))).Sum(d => d.RejectedQty),
                TotalExpired = g.GRNItems.Where(d => (d.Rack.Name.ToLower().Contains("e1") || d.Rack.Name.ToLower().Contains("expired") || d.Rack.Name.ToLower().Contains("damaged") || d.Rack.Name.ToLower().Contains("rejected") || d.Rack.Name.ToLower().Contains("purged") || (d.Rack.Description != null && (d.Rack.Description.ToLower().Contains("expired") || d.Rack.Description.ToLower().Contains("damaged") || d.Rack.Description.ToLower().Contains("rejected") || d.Rack.Description.ToLower().Contains("purged"))))).Sum(d => d.RejectedQty)
            });
