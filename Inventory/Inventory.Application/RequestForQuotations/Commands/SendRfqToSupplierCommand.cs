...
public record SendRfqToSupplierCommand(Guid Id, string UserEmail) : IRequest<bool>;

public class SendRfqToSupplierCommandHandler : IRequestHandler<SendRfqToSupplierCommand, bool>
{
    ...
    public SendRfqToSupplierCommandHandler(
    ...
    public async Task<bool> Handle(SendRfqToSupplierCommand request, CancellationToken ct)
    {
        ...
        // Verify status can be sent (from Draft or Sent)
        if (rfq.Status != RfqStatus.Draft && rfq.Status != RfqStatus.Sent)
        {
            throw new Exception("Only Draft or Sent RFQs can be sent to the supplier.");
        }

        // Fetch company and supplier details for PDF/Email
        ...
        {
            // Generate HTML layout on main thread to avoid DbContext disposal issues in background thread
            string rfqHtml = GenerateRfqHtml(company, supplier, rfq);
            string supplierEmail = supplier.Email;
            string rfqNo = rfq.RfqNo;
            string supplierName = supplier.Name;

            // Fire-and-forget background task for PDF generation & email sending
            _ = Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    try
                    {
                        pdfBytes = pdfService.Convert(rfqHtml);
                    }
                    catch (Exception pdfEx)
                    {
                        Console.WriteLine($"[SendRfqToSupplierHandler] PDF generation failed: {pdfEx.Message}");
                    }

                    await emailService.SendRfqEmailAsync(company, supplierEmail, rfqNo, supplierName, pdfBytes);
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[SendRfqToSupplierHandler] Background email sending failed: {ex.Message}");
                }
            });
        }
        else
        {
            Console.WriteLine($"[SendRfqToSupplierHandler] Skipping email. Company or Supplier or Supplier Email is null. Supplier Email: {supplier?.Email}");
        }
