...
public class ChangePasswordHandler : IRequestHandler<ChangePasswordCommand, Result<bool>>
{
    ...
    {
        var user = await _userRepository.GetByEmailAsync(request.Email, _currentUserService.CompanyId);
        if (user == null)
            return Result<bool>.Failure("User not found in this company context");
