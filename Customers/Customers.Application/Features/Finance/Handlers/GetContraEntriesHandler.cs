using MediatR;
using Customers.Application.Common.Interfaces;
using Customers.Application.DTOs;
using Customers.Application.Features.Finance.Queries;
using System.Collections.Generic;
using System.Threading;
using System.Threading.Tasks;

namespace Customers.Application.Features.Finance.Handlers
{
    public class GetContraEntriesHandler : IRequestHandler<GetContraEntriesQuery, List<ContraEntryDto>>
    {
        private readonly IFinanceRepository _repository;

        public GetContraEntriesHandler(IFinanceRepository repository)
        {
            _repository = repository;
        }

        public async Task<List<ContraEntryDto>> Handle(GetContraEntriesQuery request, CancellationToken cancellationToken)
        {
            return await _repository.GetContraEntriesAsync(request.BranchId);
        }
    }
}
